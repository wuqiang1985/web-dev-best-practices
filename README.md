...
```
/install web-dev-best-practices
```
...
在 Claude Code 会话中说 `configure` 或 `安装 rules`，会启动交互式向导，引导你：
1. 选择安装级别（用户级 / 项目级）
...
```bash
git clone https://github.com/<your-username>/web-dev-best-practices.git
...
|---|---|---|---|
| 安装方式 | `/install` + `configure` | `./install.sh` | 手动 `cp` |
| Agents/Commands/Skills/Hooks | 自动生效 | 复制到目标目录 | 按需复制 |
| Rules | 需通过 `configure` 补装 | 自动复制 | 按需复制 |
...
Skills 提供深度参考资料，在需要时按需激活。

| 技能 | 激活场景 |
|------|---------|
| `react-patterns` | 编写 React 组件、自定义 hooks、状态管理、性能优化 |
| `react-testing` | 编写组件测试、E2E 测试、配置 Vitest/Playwright |
| `api-design` | 设计 REST API、JWT 认证、分页、错误处理 |
| `postgres-patterns` | 编写 SQL 查询、设计索引、优化性能、配置 RLS |
| `redis-patterns` | 缓存策略、分布式锁、消息队列、会话管理 |
| `docker-patterns` | Dockerfile 多阶段构建、Docker Compose、安全加固 |
| `deployment-patterns` | GitHub Actions CI/CD、蓝绿部署、回滚策略 |
| `configure` | 交互式 Rules 安装向导 |
...
在 Claude Code 会话中使用 `/` 前缀调用：

| 命令 | 说明 | 示例 |
|------|------|------|
| `/plan` | 功能规划 | `/plan 实现用户认证模块` |
| `/review` | 代码审查 | `/review` （审查当前变更） |
| `/tdd` | 测试驱动开发 | `/tdd 实现购物车功能` |
| `/security-scan` | 安全扫描 | `/security-scan` |
| `/db-review` | 数据库审查 | `/db-review` |

---

## Agents 专业代理

可通过 Agent tool 或快捷命令自动调用：

| 代理 | 模型 | 职责 |
|------|------|------|
| `code-reviewer` | Sonnet | 通用代码审查（安全、质量、可维护性） |
| `react-reviewer` | Sonnet | React 专项（组件设计、hooks、性能、a11y） |
| `security-reviewer` | Sonnet | 安全审查（OWASP、密钥、注入） |
| `database-reviewer` | Sonnet | 数据库审查（SQL 质量、索引、N+1） |
| `planner` | Opus | 功能规划（需求分析、架构设计、任务拆解） |
| `tdd-guide` | Sonnet | TDD 引导（测试优先、覆盖率保障） |
...
| `pre-push-review` | 执行 git push 前 | 提醒运行 `/review` |
